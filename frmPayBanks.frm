...
If optSuppliers.value = True Then
   ...
   ' STRFORMULA = "{d_Payroll.NPay} > 0 and {d_Payroll.Bank} = '" & cboBank & "' and month({d_Payroll.EndofPeriod})=" & month(dtpEndPeriod) & " AND year({d_Payroll.EndofPeriod}) =" & year(dtpEndPeriod)
    STRFORMULA = "{d_Milkintake.QSupplied} > 0 and {d_Milkintake.LOCATION} = '" & cboBank & "' and month({TransDate.EndofPeriod})=" & month(dtpEndPeriod) & " AND year({TransDate.EndofPeriod}) =" & year(dtpEndPeriod)
...
strSQL = "SET dateformat dmy SELECT DISTINCT LTRIM(LOCATION) AS LOCATION"
strSQL = strSQL & " From d_Milkintake WHERE (QSupplied > 0) AND (LOCATION <> '')AND TransDate = '" & dtpEndPeriod & "'"
