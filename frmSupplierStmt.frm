...
'                            End If
                    ...
                    Set rst = oSaccoMaster.GetRecordset(sql)
                        ...
                        While Not rst.EOF
                            'MsgBox rs!QSupplied
                            Broughttot = 0
                           If rst!Remarks = "Brought Forward" Then
                                If Coun = 0 Then
                                  Set rstcheck1 = New ADODB.Recordset
                                  'sql = "set dateformat dmy SELECT Distinct Description FROM d_supplier_deduc WHERE SNo = '" & txtSNo & "' AND Description='" & rst!description & "' AND Remarks='Brought Forward' AND Date_Deduc>='" & Startdate & "' AND Date_Deduc<='" & Enddate & "'"
                                  sql = "set dateformat dmy SELECT Distinct Description FROM d_supplier_deduc WHERE SNo = '" & txtSNo & "' AND Remarks='Brought Forward' AND Date_Deduc>='" & Startdate & "' AND Date_Deduc<='" & Enddate & "'"
                                  Set rstcheck1 = oSaccoMaster.GetRecordset(sql)
                                While Not rstcheck1.EOF
                                   Set rstcheck = New ADODB.Recordset
                                   
                                      sql = "d_sp_PrintDeductStmtTotal '" & txtSNo & "','" & rstcheck1!description & "','" & Startdate & "','" & Enddate & "'"
                                      Set rstcheck = oSaccoMaster.GetRecordset(sql)
                                      If rstcheck.Fields(0) <> 0 Then
                                         txtFile.WriteLine Startdate & " " & vbTab & " " & Format(rstcheck.Fields(0), "#,##0.00" & vbTab & " " & rstcheck1!description & " 'Brought Forward' ")
                                         Broughttot = rstcheck.Fields(0) + Broughttot
                                         Coun = Coun + 1
                                       'GoTo movetonext
                                       End If
                            ...
                            Else
                               ...
                               TotDeduction = TotDeduction + rst!amount
                            End If
                            TotDeduction = TotDeduction + Broughttot
                           'txtfile.WriteLine rs!PPU
                             rst.MoveNext
                          Wend
                        Set rst1 = New ADODB.Recordset
