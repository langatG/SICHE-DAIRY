...
Begin VB.Form frmproductrepackaging 
   ...
   Begin VB.ComboBox Cmbstation 
      Height          =   315
      ItemData        =   "frmproductrepackaging.frx":0000
      Left            =   4320
      List            =   "frmproductrepackaging.frx":0007
      TabIndex        =   48
      Top             =   4080
      Width           =   1935
   End
   ...
   Begin VB.PictureBox Picture2 
      ...
      Height          =   360
      Left            =   3480
      Picture         =   "frmproductrepackaging.frx":0018
      ScaleHeight     =   360
      ScaleWidth      =   240
      TabIndex        =   8
      Top             =   120
   ...
   Begin VB.ComboBox cbosupplier 
      Height          =   315
      ItemData        =   "frmproductrepackaging.frx":019A
      Left            =   1560
      List            =   "frmproductrepackaging.frx":019C
      TabIndex        =   6
      Top             =   1560
      Width           =   4455
   End
   ...
   Begin VB.Frame frmnewit 
      ...
      Top             =   360
      Width           =   6135
      Begin VB.ComboBox Cmbstation2 
         Height          =   315
         ItemData        =   "frmproductrepackaging.frx":019E
         Left            =   4320
         List            =   "frmproductrepackaging.frx":01A5
         TabIndex        =   46
         Top             =   3600
         Width           =   1455
      End
      ...
      Begin VB.PictureBox Picture8 
         ...
         Height          =   360
         Left            =   3720
         Picture         =   "frmproductrepackaging.frx":01B6
         ScaleHeight     =   360
         ScaleWidth      =   240
         TabIndex        =   35
         Top             =   360
...
End If
 ...
 sql = "SELECT     UserLoginIDs, UserGroup, Branch,SUPERUSER From UserAccounts where UserLoginIDs='" & User & "'"
    ...
    MsgBox "You are not allowed to Do Repackage in this branch", vbInformation
    Exit Sub
    End If
    End If
...
sql = "select p_code, S_No, Qout, sprice,pprice,SupplierID,AI from ag_products where p_name='" & cboproductname & "' and p_code='" & txtpcode & "'"
...
Set rst = oSaccoMaster.GetRecordset("select p_code ,Qout from ag_products where p_name ='" & cboproductname1 & "'")
' And p_code = " & txtpcode1 & " ")
If Not rst.EOF Then
txtpcode1 = rst.Fields("p_code")
'txtcurrstock = rst.Fields("Qout")
End If
End Sub
...
'SELECT p_code, p_name, S_No, Qout, sprice FROM   ag_Products
'sql = "select p_code, S_No, Qout, sprice from ag_products where p_name='" & cboproductname & "'"
sql = "select p_code, S_No, Qout, sprice,AI from ag_products where p_name='" & cboproductname1 & "'"
Set rs = New ADODB.Recordset
Set rs = oSaccoMaster.GetRecordset(sql)
If Not rs.EOF Then
...
cn.Open Provider, "atm", "atm"
'If reportpath = "" Then reportpath = GetSetting("payroll", "AppName", "rptPath", rptPath)
sql = "select P_CODE,qout,unserialized,AI from ag_products where p_code='" & txtpcode1 & "'"
Set rs = New ADODB.Recordset
rs.Open sql, cn
If rs.EOF Then
...
sql = ""
sql = "set dateformat dmy insert into  ag_products(p_code,p_name,s_no,qin,qout,date_entered,last_d_updated,user_id,audit_date,o_bal,supplierid,serialized,unserialized,seria,pprice,sprice,Expirydate,Branch,AI )"
sql = sql & "  values('" & txtpcode1.Text & "','" & cboproductname1.Text & "'," & txtSERIALNO.Text & "," & txtnewpro.Text & "," & txtnewpro.Text & ",'" & txtdateenterered.value & "','" & txtdateenterered.value & "','Admin','" & Date & "'," & txtnewpro.Text & ",'" & cbosupplier & "',0," & unsera & "," & seria & "," & txtnpp & "," & txtNsp & "," & DTPexdate.value & ",'" & Cmbstation2 & "'," & lblai & ")"
cn.Execute sql


If txtNsp = "" Then txtNsp = 0
If txtnpp = "" Then txtnpp = 0

sql = ""
sql = "set dateformat DMY INSERT INTO ag_stockbalance"
sql = sql & " (p_code, productname, openningstock, changeinstock, stockbalance, transdate,companyid,pprice,sprice,RLevel,Expirydate,Branch,AI)"
sql = sql & " VALUES     ('" & txtpcode1.Text & "','" & cboproductname1 & "', " & txtnewpro & ", " & txtnewpro & ", " & txtnewpro.Text & ", '" & txtdateenterered & "',1," & txtnpp & "," & txtNsp & "," & txtRLevel & "," & DTPexdate.value & ", '" & Cmbstation2 & "'," & lblai & ")"
cn.Execute sql
Else
'update curr stock'
'sql = ""
...
sql = ""
sql = "set dateformat DMY INSERT INTO ag_stockbalance"
sql = sql & " (p_code, productname, openningstock, changeinstock, stockbalance, transdate,companyid, Pprice, Sprice,Branch,AI)"
sql = sql & " VALUES     ('" & txtpcode1 & "', '" & cboproductname1 & "', '" & txtnewpro & "', '" & txtnewpro & "', '" & txtnewpro.Text + rs.Fields("qout") & "', '" & txtdateenterered & "',1, " & txtnpp & "," & txtNsp & ", '" & Cmbstation2 & "'," & lblai & ")"
cn.Execute sql
...
'update curr stock'
sql = ""
sql = "set dateformat DMY update ag_products set p_name='" & cboproductname & "',qin=" & txtbalance.Text & ",qout=" & txtbalance.Text - txtquantity & ",o_bal=" & txtbalance.Text & ",last_d_updated='" & Date & "',user_id='" & User & "',audit_date='" & Date & "',unserialized=" & unsera & ",SERIA=" & seria & ",pprice=" & txtpprice & ",sprice=" & txtsellingprice & " where p_code='" & txtpcode.Text & "'"
cn.Execute sql

'Else
'sql = ""
'sql = "set dateformat DMY Update ag_stockbalance SET   productname = '" & cboproductname & "', openningstock = " & txtbalance & "-1, changeinstock = " & txtbalance & "-1, stockbalance = " & txtbalance.Text & ", transdate = '" & txtdateenterered & "' WHERE     (p_code = '" & txtpcode & "') "
'
'cn.Execute sql

sql = ""
sql = sql & "SET dateformat DMY INSERT INTO ag_Receipts(R_No, P_code, T_Date, Amount,S_no, Qua, S_Bal, user_id, Cash, SNo,Transby, Idno, mobile,branch,remarks,SPRICE,BPRICE,AI) VALUES ( '','" & txtpcode & "','" & txtdateenterered & "'," & txtsellingprice & ",''," & txtquantity & ",'','" & User & "','','','','','','" & Cmbstation & "','Repackage','" & txtsellingprice & "','" & txtpprice & "'," & lblai & ")"
cn.Execute sql
'oSaccoMaster.ExecuteThis (sql)
'oSaccoMaster.ExecuteThis ("Update ag_Receipts SET S_Bal =" & CCur(Remain) & " WHERE p_code= '" & txtpcode & "'")
'//XX
sql = "set dateformat dmy insert into  ag_products3(p_code,p_name,qin,qout,date_entered,last_d_updated,user_id,audit_date,o_bal,supplierid,serialized,unserialized,seria,pprice,sprice,branch,AI )"
sql = sql & "  values('" & txtpcode1.Text & "','" & cboproductname1.Text & "'," & txtnewpro.Text & "," & txtnewpro.Text & ",'" & txtdateenterered.value & "','" & txtdateenterered.value & "','" & User & "','" & Date & "'," & txtnewpro.Text & ",'',0," & unsera & "," & seria & "," & txtnpp & "," & txtNsp & ",'" & Cmbstation2 & "'," & lblai & ")"
cn.Execute sql
...
DTPexdate = Format(Date, "dd,mm,yyyy")
...
While Not rs.EOF
   ...
   ' Dim cn As Connection
    ...
    rst.Open sql, cn, adOpenKeyset, adLockOptimistic
    While Not rst.EOF
    Cmbstation2.AddItem rst.Fields(0)
    rst.MoveNext
    Wend
...
Set cn = New ADODB.Connection
 cn.Open Provider, "atm", "atm"
'If reportpath = "" Then reportpath = GetSetting("payroll", "AppName", "rptPath", rptPath)
sql = "select P_CODE,P_NAME,S_NO,QOUT,supplierID,pprice,sprice,AI from ag_products where Branch='" & Cmbstation & "' and p_code='" & Y & "'"
Set rs = New ADODB.Recordset
rs.Open sql, cn
If Not rs.EOF Then
...
If Not IsNull(rs.Fields(7)) Then lblai = (rs.Fields(7))
...
rs.Open sql, cn
If Not rs.EOF Then
If Not IsNull(rs.Fields(0)) Then txtpcode1 = (rs.Fields(0))
If Not IsNull(rs.Fields(1)) Then cboproductname1 = (rs.Fields(1))
'If Not IsNull(rs.Fields(4)) Then cbosupplier = (rs.Fields(4))
If Not IsNull(rs.Fields(5)) Then txtnpp = (rs.Fields(5))
If Not IsNull(rs.Fields(6)) Then txtNsp = (rs.Fields(6))
'If Not IsNull(rs.Fields(2)) Then txtserialno = (rs.Fields(2))
If Not IsNull(rs.Fields(3)) Then txtcurrstock = (rs.Fields(3))

If txtcurrstock <= 0 Then
MsgBox "Your stock is below zero please reorder", vbInformation
End If
'// check with serial no if it exist
...
Private Sub txtcurrstock_Change()
If Cmbstation2 = "" Then
MsgBox "Please select the branch", vbInformation
Exit Sub
End If
...
End Sub
Private Sub txtpcodeRe_Change()
'//TWNG001
Provider = "MAZIWA"
Set cn = New ADODB.Connection
 cn.Open Provider, "atm", "atm"
'If reportpath = "" Then reportpath = GetSetting("payroll", "AppName", "rptPath", rptPath)
sql = "select P_CODE,P_NAME,S_NO,QOUT,supplierid,pprice, sprice from ag_products where p_code='" & txtpcode1 & "'"
Set rs = New ADODB.Recordset
rs.Open sql, cn
If Not rs.EOF Then
 txtpcode1 = (rs.Fields(0))
...
Private Sub txtpcode1_KeyPress(KeyAscii As Integer)
On Error GoTo HEREEE
If KeyAscii = 13 Then
txtpcodeRe_Change
Else
