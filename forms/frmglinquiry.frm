...
AvailableBal = 0
Dim ssql As String
    ssql = "SELECT * FROM   GLTRANSACTIONS where craccno='" & txtAccNo & "'  order by TRANSDATE ASC,id ASC"
    ...
    With lvememtrans
      If Rs!transdate <> "" Then
       Set lv = .ListItems.Add(, , Rs!transdate)
        ...
        If Rs!amount <> "" Then
          If UCase(Trim(Rs!Craccno)) = "CR" Then
        ...
        Else
        If Rs!amount <> "" Then
          If UCase(Trim(Rs!DRaccno)) = "DR" Then
            lv.ListSubItems.Add , , Format(Rs!amount, "###,###,###.00")
            lv.ListSubItems.Add , , Format(0, "0.00")
            AvailableBal = AvailableBal - amount
          Else
        ...
        If Not IsNull(Rs!amount) Then
            If Check_if_asset(txtAccNo) = True Then
             lv.ListSubItems.Add , , Format(Format((amount), "###,###,###.00"), "###,###,###.00")
            Else
            lv.ListSubItems.Add , , Format((amount), "###,###,###.00")
            End If
        Else
             lv.ListSubItems.Add , , "0.00"
        End If
   
         If Rs!TransDescript <> "" Then
              lv.ListSubItems.Add , , Rs!TransDescript
         Else
